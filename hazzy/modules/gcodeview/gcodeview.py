...
LANGDIR = os.path.join(PYDIR, 'gcode_highlight')
...
class GcodeView(object):
    ...
    def __init__( self ):
        ...
        path = self.sm.get_search_path().append(LANGDIR)
        self.sm.set_search_path(path)
        path = self.lm.get_search_path().append(LANGDIR)
        ...
        #self.sm.set_search_path( [LANGDIR] )
